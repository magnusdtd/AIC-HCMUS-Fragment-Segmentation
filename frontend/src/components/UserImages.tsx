...
function UserImages() {
  ...
  if (error) {
    return (
      <div className='flex items-center justify-center h-screen'>
        <h1 className='text-xl text-red-700'>{error}</h1>
      </div>
    );
  }
  ...
  if (images.length === 0) {
    return (
      <div className='flex items-center text-2xl font-sans justify-center h-screen'>
        <h1 className='text-xl text-red-700'>No images found. Start uploading your images!</h1>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <h2 className="text-2xl font-bold mt-4">Your Images</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {images.map((image, index) => (
          <Card key={index} sx={{ maxWidth: 512 }} className="m-2">
            <CardContent>
              {image.url ? (
                <img
                  src={image.url}
